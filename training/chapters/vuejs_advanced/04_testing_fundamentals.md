...
- **Introduce** a test suite into a project that has none: what to test first,
  characterization tests, and a coverage ratchet
...
| 4 | **One end-to-end journey** (chapter 8) | login → the main task → logout. Catches the integration breaks the unit tests never will |
...
- Untestable component? That is a **design** answer, not a testing one: pull the
  logic into a composable (chapter 3quater, step 2), then test the composable
...
- On an existing project: pure functions first, then stores and composables, then
  the components that keep breaking — never a coverage target
- Pin legacy behaviour with characterization tests, and ratchet coverage with
  `thresholds.autoUpdate` instead of setting a number nobody can reach
...
- Add a coverage **ratchet** — `thresholds: { autoUpdate: true }` — commit the
  numbers, then watch them go up on the next run
- *(Bonus, on your own project)* List the ten files with the most commits
  (`git log --format= --name-only | sort | uniq -c | sort -rn | head`), and write
  a characterization test on the first one you can mount
