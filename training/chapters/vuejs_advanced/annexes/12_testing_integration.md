...
# Annexe — Testing in integration & e2e
...
Chapter 3 tested components and composables **in isolation**. Since then you have
added the two things that make a component hard to isolate:

- a **router** (chapter 6) — `useRoute`, `useRouter`, guards, redirects
- a **store** (chapter 9) — shared state that survives the component
...
- `global.provide` is the test-side counterpart of the `provide` / `inject` pair —
  same injection key, injected value under your control
...
Chapter 3 stubbed `InvoiceChart` for exactly one reason:
...
- This is the demo in TP 3/12 — `npm run test:browser`
...
- This job is the one you will wire into the pipeline of chapter 16
...
## Workshop 12 - Testing in integration & e2e (part 2 of the testing workshop)
