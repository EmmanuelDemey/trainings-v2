...
# Annexe — A team composable library
...
- The library **never** imports from an application. If it needs a value from one,
  the value is **injected** (chapter 4)
...
// 2. Per app — provided by a plugin, read through inject  (chapter 4)
...
- A composable is **never** `async`. It returns refs that fill in later — that is
  what `Suspense` (chapter 7) is for on the component side
...
- Chapter 3 covers the tooling; here the point is that these four are the
...
## Workshop 5 - Extracting the library — 45 min
