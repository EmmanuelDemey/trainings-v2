...
**Part 1 (steps 1a, 3, 5, 7) is the chapter-3 workshop**: components and
composables **in isolation**.

**Part 2 (steps 1b, 2, 4, 6, 8) is optional** — the same material as
`tp/12_testing_integration/`, on the app you already know, for a group that
prefers one codebase end to end. The chapter-12 workshop of record is the other
one; do one or the other, not both. If you do come back here, the tests you wrote
in part 1 must still be green.
...
For the browser mode demo (chapter 12, see below):
...
# Part 2 — after chapter 12 (integration & e2e)

> **The chapter-12 workshop of record is `tp/12_testing_integration/`** — a
> separate, autonomous project that starts already unit-tested. Part 2 below
> stays for a group that prefers to carry one codebase end to end; it is the same
> material on the app you already know. Do one or the other, not both.
...
- Browser mode gets its **own** `vitest.browser.config.ts`, so the everyday
  ...
  two as `test.projects` in a single config (chapter 12 shows how).
