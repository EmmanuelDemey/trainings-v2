...
const routes: RouteRecordRaw[] = [
  ...
  },
  {
    ...
    // TODO 2.1: this list must require authentication.
    meta: { title: 'Invoices' },
  },
  {
    path: '/invoices/:id(\\d+)',
  ...
  },
  {
    ...
    // TODO 3.1: restrict this route to the `admin` role via `meta.roles`.
...
export const router = createRouter({
  ...
  // TODO 4.1: implement `scrollBehavior(to, from, savedPosition)`:
...
// TODO 2.2: global `beforeEach` guard.
...
//
// TODO 3.2: extend the guard with a role check based on `meta.roles`, and
//   redirect to `{ name: 'forbidden' }` when it fails.
//
// TODO 5.1: add an `afterEach` that sets `document.title` from `to.meta.title`.
//
// TODO 5.2 (bonus): add a direction-aware transition. In an `afterEach`, compare
//   the depth of `to.path` and `from.path` and set `to.meta.transition` to
//   'slide-left' or 'slide-right'.

// `meta.transition` is read by App.vue's <RouterView>, which is already wired.
