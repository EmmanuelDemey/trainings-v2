...
Optional 90-minute slot at the end of day 3, or a follow-up session two weeks
later. The follow-up works better, and it is the one that can afford the half-day
format below: people come back with questions from their own codebase, and the
review round has somewhere to land.
...
|---|---|
| Freeze at 0:50 | Pairs keep coding, reviewers review a moving target, the round dies |
| "Run before read" at 0:55 | Reviewers open the editor first, spend the whole round on naming |
| Restitution at 1:20 | Authors start defending, the second pair never gets its 2 minutes |
...
only thing that keeps two minutes to two minutes.

**Longer version (½ day), for a follow-up session.** 1h45 of build, 45 min of
review, 20 min of restitution and 15 min of debrief, with the freeze at 2:00 and
the restitution at 2:55; steps 1, 2, 3 and 5 come into the Definition of Done.

In the 90-minute slot, only **steps 1 and 2** do. Say so at the start and drop
the rest from the Definition of Done out loud — a pair that discovers mid-way
that half the brief was decoration stops trusting the brief.
...
| 2 | `src/components/InvoiceTable.vue` | `v-for … :key="index"` | Filter, then change a status: the wrong row updates. Reactivity basics, and the one people *say* they know. |
...
was on?"). A finding someone reaches themselves in the debrief is worth three you
announced during the restitution.
...
|---|---|
| "Your `loading` flickers" | The `finally` guard. Chapter 2, and the reason the spec has that test. |
...
| "You catch every error in the boundary" | What a boundary does *not* catch: event handlers, `setTimeout`. Chapter 13. |
...
Ten minutes together with the restitution — fifteen on its own in the half-day
version — and the four questions in the README. The one that pays:
