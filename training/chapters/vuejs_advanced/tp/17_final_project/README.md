# TP 17 — Final project (optional, 90 min, in pairs, with cross-review)

> **Optional.** This workshop is the one place in the training where nothing new
> is taught. You take chapters 2 to 16, build one vertical slice of a real app
> with them, hand it to another pair, and review theirs. Ninety minutes, two
> roles, one deliverable each.
...
- Chapters 2 to 16. Workshops 2 to 16 are not required, but everything here has
...
|---|---|---|
| 0:00 | 10 min | Read this file **together**, agree on which steps you attack and in which order |
| 0:10 | 40 min | **Build.** Driver / navigator, swap every 20 minutes |
| 0:50 | 5 min | **Freeze.** `npm run review`, then write the handover note (below) |
| 0:55 | 25 min | **Cross-review.** You review another pair's work while they review yours |
| 1:20 | 10 min | **Restitution** and group debrief with the trainer |

The freeze at 0:50 is not negotiable. A reviewer cannot review a moving target,
...
every 20 minutes, driver becomes navigator, no exception — including in the
middle of something.
...
Seven steps, and 40 minutes to build. **Steps 1 and 2 are the core** — a pair
that does those two well has something worth reviewing. Step 3 is where a fast
pair goes next; steps 4 to 7 are where the slice goes **on your own, after the
...
## The cross-review round — 25 minutes
...
**The 25 minutes, in order — the order matters:**

| | |
|---|---|
| **12 min — run it** | Before reading a single line. `npm run review`, then walk the journeys in the Definition of Done above. Write down what you observe, not what you suspect. |
| **8 min — read it** | Only now. Their `HANDOVER.md` first, then their tests, then the two files it pointed you at. You will not read everything — pick with the grid below. |
| **5 min — write it** | Three findings, maximum. Plus one thing you are keeping for your own project. |
...
they will eat your 25 minutes if you let them.

### Restitution — 10 minutes

Two minutes per pair, out loud, to the authors:
...
you reopen the project. Rerun `npm run review`. A review that changes nothing was
a conversation.
...
- Add the two Cypress journeys from chapter 12 — deep link and rollback — and make
