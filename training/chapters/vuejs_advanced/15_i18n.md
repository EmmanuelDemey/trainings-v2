...
- **B.** Only the runtime: vue-i18n fetches catalogues by itself
- **C.** The runtime + the compiler, always
- **D.** vue-i18n's runtime + the reference locale, if the others are behind
  `import.meta.glob` and loaded on demand

<v-click>

> ✅ **D** — Splitting catalogues is **your** job: `import.meta.glob` (or a dynamic
