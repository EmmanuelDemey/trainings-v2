...
const { products, status, error, loadDurationMs } = storeToRefs(catalog);
const { search } = storeToRefs(ui);
  ...
  <section>
    ...
    <div class="row" style="margin-bottom: 0.75rem">
      ...
      <button
        ...
        @click="catalog.loadProducts(size)"
      >
    ...
    <p class="muted">
      ...
      making the payload reactive, not of the network. The store uses
      <code>shallowRef</code>: switch it back to <code>ref</code>, load 30 000
    ...
    <ul class="products">
      <li v-for="product in visible" :key="product.id">
        ...
        <button type="button" :data-testid="`add-${product.id}`" @click="cart.addToCart(product.id)">
          Add
        </button>
      </li>
    </ul>
