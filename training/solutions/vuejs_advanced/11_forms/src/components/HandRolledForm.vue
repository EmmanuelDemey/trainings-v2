...
const onSubmit = handleSubmit(async (data) => {
  ...
  try {
    ...
    if (error instanceof ApiValidationError) {
      // The server keys its errors by FORM PATH (`attendees[0].name`), which is
      // the same key `errorFor` looks up — so a 422 lands on the right input
      // rather than in a banner the user has to map back to a field by hand.
