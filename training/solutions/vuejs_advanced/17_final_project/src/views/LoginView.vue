 ...
 *
 * `(?!\/)` is the part that matters: `//evil.example.com` starts with a slash
 * and the browser reads it as `https://evil.example.com`. Without the negative
 * lookahead this is an open redirect that passes every "does it start with a
 * slash?" review — and open redirects are how phishing links borrow your
 * domain's reputation.
 */
...
async function onSubmit(): Promise<void> {
  ...
  try {
    ...
    // the first Back after signing in returns to it — and the guard bounces the
    // user straight home. A loop Back cannot escape.
