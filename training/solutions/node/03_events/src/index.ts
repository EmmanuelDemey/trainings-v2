...
orders.onPaid((order: Order, paidAmount: number) => {
  console.log('[order:paid]', order.id, `${paidAmount} ${order.currency}`);
});

const order: Order = { id: 'ord_1024', amount: 49.9, currency: 'EUR' };
orders.created(order);
orders.paid(order, order.amount);
