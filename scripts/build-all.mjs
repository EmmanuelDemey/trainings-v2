...
//       javascript-participants.zip both PDFs + the workshop folders, no solutions
...
import { NOT_SHIPPED, participantsZip } from './participants-zip.mjs';
...
/** Zips a folder, skipping what an install/build regenerates and local env overrides. */
...
for (const training of TRAININGS) {
  ...
  // The participant kit: both PDFs above and the workshop folders, without the
  // solutions. Last, because it reads the two PDFs this loop just produced.
  if (existsSync(workshopsDir)) {
    const bytes = await participantsZip({ slug: training.slug, workshopsDir, downloadsDir });
    console.log(`  ${training.slug}-participants.zip  ${(bytes / 1024 / 1024).toFixed(1)} MB`);
  }
}
